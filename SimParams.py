...
class sim_setup:
    ...
    def set_lat_occ(self,method:str,lat_template=np.empty((1,1)),**kwargs):
        ...
        if method.lower() == 'random':
            ...
            sum_tot = sum(theta_key.values())
            if not np.isclose(sum_tot,1.0,atol=0.01):
                raise ValueError('Total fractional coverage must be 1, note the empty site coverage is also required',sum_tot,theta_key)
